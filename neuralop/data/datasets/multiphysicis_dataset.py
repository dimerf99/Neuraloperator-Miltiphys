...
from ..transforms.data_processors import DefaultDataProcessor, MultiphysicsDataProcessor
...
class GridPreprocessor:
    ...
    def resize_to_common_grid(self, data, target_resolution):
        _, *res = data.shape
        if torch.all(torch.tensor(res) == target_resolution):
            return data
...
class MultiphysicsDataset:
    ...
    def __init__(self,
        ...
        data["x"] = self.grid_preprocessor.resize_to_common_grid(data["x"], train_resolution)
        data["y"] = self.grid_preprocessor.resize_to_common_grid(data["y"], train_resolution)
        ...
        x_train = self.grid_preprocessor.subsample(x_train, input_subsampling_rate, n_train, channel_dim)
        y_train = self.grid_preprocessor.subsample(y_train, output_subsampling_rate, n_train, channel_dim)
        ...
        for (res, n_test) in zip(test_resolutions, n_tests):
            ...
            data["x"] = self.grid_preprocessor.resize_to_common_grid(data["x"], res)
            data["y"] = self.grid_preprocessor.resize_to_common_grid(data["y"], res)
            ...
            x_test = self.grid_preprocessor.subsample(x_test, input_subsampling_rate, n_test, channel_dim)
            y_test = self.grid_preprocessor.subsample(y_test, output_subsampling_rate, n_test, channel_dim)
